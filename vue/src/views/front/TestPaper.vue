      ...
      <div v-for="item in data.testPaperData.questions" style="margin-bottom: 20px">
        ...
        <div v-if="item.typeId === 1">
          <el-radio-group v-model="item.newAnswer">
            <el-radio value="A">A. {{ item.optionA }}</el-radio><!--绑定的值-->
            <el-radio value="B">B. {{ item.optionB }}</el-radio>
            <el-radio value="C">C. {{ item.optionC }}</el-radio><!--外面{{}}-->
            <el-radio value="D">D. {{ item.optionD }}</el-radio>
          </el-radio-group>
        </div>
        <!--   typeId === 2：多选题     -->
        <div v-if="item.typeId === 2">
          <el-checkbox-group v-model="item.checkList"><!--element用法绑定到数组-->
...
const data = reactive({
  testPaperId: router.currentRoute.value.query.id,/*单个根据序号查询，获取到的试卷的标号，注意*/
  testPaperData: {},
  hour: 0,
  minutes: 0,
  seconds: 0,
...
let time
const loadTestPaper = () => {
  data.testPaperId = router.currentRoute.value.query.id/*这段代码的作用是从 Vue Router 的路由查询参数（URL 的 ? 部分）中获取 id 的值，并将其赋值给 data.testPaperId。响应式对象就是ref或者reactive*/
...
/*提交到score里面去了，将teatpaper的信息转换为score里面去，重新填写塞到score数据库中去，其中score里面最关键的是那个answer，answer里面存储了对应的题目的id标准答案和学生的答案，其中answer是指学生的是将Answer类型的数组整合然后变成string存到数据库中的*/
const submitPaper = () => {
  clearInterval(time)
  // 提交试卷
