...
/*这里在前端判断，因为只有这里需要根据status的状态来进行return，判断不显示，平时的check，这里的item的id是在？后面，是在url展示的，然后跳转到的对应的页面在router的url？后面获取查询的id赋值，展现当前的页面*/
const navTo = (item) => {
  if (item.status === '未开始') {
  ...
  if (item.status === '已结束') {
    ...
    return;
  }/*这里的逻辑就是不能是exam.vue中请求的不一定是exam有关的，可能是其他entity的函数跳转什么的，同时*/
