...
<!--这个考试状态不在数据库里面，是在前端那里的要显示的，根据service里面的start和end进行的判断-->
    ...
    <el-dialog title="试卷信息" v-model="data.formVisible" width="40%" destroy-on-close>
      <el-form ref="form" :model="data.form" label-width="70px" style="padding: 20px">
        <el-form-item prop="courseId" label="选择课程"><!--这里@change是查阅element-plus官方网页醉后select API 的attribute和event看到的，这个选中后要立刻更新，因为下一步的手动选择题目是根据课程id来的，调用加载对应的loadquestion的函数显示该课程的对应的问题-->
...
<!--多选框multiple，这个自动把value的值变成一个数组，根据vue3官方的文档显示查阅-->
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="add">确 定</el-button><!--取消就是表格不可见，不加入数据库，如果是进行数据库的操作就是add确定-->
        </span>
      </template>
    </el-dialog>
  </div>
...
const data = reactive({/*里面是数据一种是在右侧的，传入的一种是在左侧的，注意识别*/
  user: JSON.parse(localStorage.getItem('xm-user') || '{}'),
  formVisible: false,/*默认的是false，表格不可见*/
  form: {},/*填写的表单form里面的ids和外面的ids不是同一个东西*/
  tableData: [],/*load*加载的表格的信息用来展示数据*/
  pageNum: 1,
  pageSize: 10,
  total: 0,
  name: null,
  ids: [],/*手动选择的题目的标号*/
  courseData: [],/*loadcourse需要加载的表单的下拉框select中的所有课程信息获取*/
  questionData: []/*loadquestion根据选择的course加载查询出所有的对应的课程的问题*/
  /*对应的前端要传过来的东西需要接受，用什么接受呢，在实体类中添加对应的属性，把前端用到的数据信息放在实体类中，不然无法接收显示*/
})
const loadCourse = () => {
...
const loadQuestion = (courseId) => {/*填完表单才确定的，作为输入的参数传入，而非像loadcourse一样，打开页面就是确定的虽然有参数teacherId但是是固定的，所以不用输入参数，重点在selectall中where根据不同的信息进行查询*/
...
const add = () => {
  request.post('/testPaper/add', data.form).then(res => {
    if (res.code === '200') {
      ...
      load()/*每次更新和删除过后要重新加载load()，把最新的数据库的信息加载出来*/
    } else {
      ElMessage.error(res.msg)
    }
  })
